...
$('#GoBack[href*=\\#]').on('click', function(event) {
    event.preventDefault();
    $('html,body').animate({
        scrollTop: 0
    }, 500);
...
/* MODIFICATIONS EN COURS SUR LE SITE INTERNET */

jQuery(document).ready(function()
{
    jQuery("#icone_moins").hide();
    jQuery("#texte_abord").hide();
   // toggle() lorsque le lien avec l'ID plus est cliqué
    jQuery("#icone_plus").click(function()
                                
  {
      jQuery("#texte_abord").slideToggle();
      jQuery("#icone_plus").hide();
      jQuery("#icone_moins").show();  
      return false;
    
   });
    jQuery("#icone_moins").click(function()
  {
      jQuery("#texte_abord").toggle("slow");
      jQuery("#icone_moins").show(); 
      jQuery("#icone_plus").show();
      jQuery("#icone_moins").hide();
      return false;
